...
class AppLogger(GetLogger):
    ...
    def __init__(self):
        try:
            if not settings.APP_LOG_FOLDER.exists():
                os.mkdir(settings.APP_LOG_FOLDER)
        except Exception as error:
            raise Exception(error)

        super().__init__(
            log_file=settings.APP_LOG_FILE,
            logger_name=settings.APP_LOGGER_NAME,
            log_level=settings.APP_LOGGING_LEVEL
        )
